...
resource "aws_iam_role" "main" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [
      ...
      # Self-assuming — required by Unity Catalog storage credentials.
      # Uses account root (not the role ARN) because the role doesn't
      # exist yet at creation time and AWS rejects unknown principals.
      {
        Effect = "Allow"
        Action = "sts:AssumeRole"
        Principal = {
