...
# self-service data generator writes to BOTH PostgreSQL (customer, hotel via CDC)
# and Kafka directly (clickstream, bookings, reviews with Avro schemas).
...
locals {
  # Only deploy data generator in self-service mode (not WSA shared mode)
...
# ===============================
# Data generator connects to PostgreSQL on localhost (co-located on same EC2).
...
# ===============================
# Data generator writes clickstream, bookings, and reviews
...
# configs to the EC2 instance, then starts the data generator as a Docker container.
...
resource "null_resource" "datagen_setup" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      set -e
      echo '================================================'
      echo 'Data Generator Setup (Self-Service)'
      echo '================================================'
