...
- **Lab content**: Resolved broken image links; added `AI_SENTIMENT` availability warning; prefixed 3 topics with `riverhotel.cdc.`
...
- **Tableflow DLQ**: Added detailed steps for enabling `Log` error mode to a Tableflow-enabled topic

### Fixed

- **Outdated references**: Fixed a few references to behavior or names that were missed in the previous PR
