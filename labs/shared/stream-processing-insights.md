...
### A3: CDC Timestamp Handling

The CDC connector is configured with `time.precision.mode = connect` and PostgreSQL uses `TIMESTAMP` (without time zone) columns. This means timestamps arrive in Flink as `TIMESTAMP(3)` (millisecond precision) and can be used directly in watermark definitions without conversion.
